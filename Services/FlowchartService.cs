...
internal class FlowchartService
{
    ...
    public static string GenerateSvg(LuaArea area, DataService? ds)
    {
        ...
        // 7. Calculate node dimensions
        foreach (var n in nodes.Values)
            CalculateNodeSize(n);

        // 8. Collect real edges BEFORE dummy insertion modifies Children/Parents
    ...
    private static List<List<string>> SugiyamaLayout(
        Dictionary<string, FNode> nodes)
    {
        AssignLayers(nodes);
    ...
    private static void AssignLayers(Dictionary<string, FNode> nodes)
    {
        ...
        var usedLayers = nodes.Values.Select(n => n.Layer).Distinct().OrderBy(l => l).ToList();
        if (usedLayers.Count > 0)
        {
            var remap = new Dictionary<int, int>();
            for (int i = 0; i < usedLayers.Count; i++)
                remap[usedLayers[i]] = i;
            foreach (var n in nodes.Values)
    ...
    /// <summary>
    /// Post-pass: aligns linear chains (single-parent/child sequences) to the same X center
    /// and shifts multi-child parents to align with their first logical child.
    /// Eliminates gradual drift in linear sequences and straightens primary edges.
    /// </summary>
    private static void AlignLinearChains(Dictionary<string, FNode> nodes, List<List<string>> layers)
    {
        // Build layer lookup for overlap checking
        var layerOf = new Dictionary<string, int>(StringComparer.Ordinal);
        for (int li = 0; li < layers.Count; li++)
            foreach (var nid in layers[li])
                layerOf[nid] = li;

        // Step A: Detect and align linear chains
        var visited = new HashSet<string>(StringComparer.Ordinal);
        foreach (var n in nodes.Values)
        {
            if (n.IsDummy || visited.Contains(n.Id)) continue;
            // Start of a chain: node with != 1 parent, but has exactly 1 real child
            int realParentCount = n.Parents.Count(pid => nodes.ContainsKey(pid) && !nodes[pid].IsDummy);
            if (realParentCount == 1)
            {
                // Check if the single parent has only 1 real child (this node) → true continuation, skip
                var parentId = n.Parents.First(pid => nodes.ContainsKey(pid) && !nodes[pid].IsDummy);
                var parent = nodes[parentId];
                int parentRealChildren = parent.Children.Count(cid => nodes.ContainsKey(cid) && !nodes[cid].IsDummy);
                if (parentRealChildren == 1) continue;
                // Parent has multiple children → this node is a branch start; allow as chain start
            }

            // Walk the chain from this node
            var chain = new List<FNode>();
            var cur = n;
            while (cur != null && !visited.Contains(cur.Id))
            {
                ...
                // Find the single real child
                var realChildren = cur.Children
                    .Where(cid => nodes.TryGetValue(cid, out var c) && !c.IsDummy)
                    .ToList();
                if (realChildren.Count != 1) break;

                var child = nodes[realChildren[0]];
                // Child must have exactly 1 real parent (this node)
                var childRealParents = child.Parents
                    .Where(pid => nodes.TryGetValue(pid, out var p) && !p.IsDummy)
                    .ToList();
                if (childRealParents.Count != 1) break;
            ...
            // Align chain to the first node's center X
            double targetCenterX = chain[0].X + chain[0].Width / 2;
            for (int i = 1; i < chain.Count; i++)
            {
                double shift = targetCenterX - (chain[i].X + chain[i].Width / 2);
                if (Math.Abs(shift) < 1) continue;

                // Check overlap constraints in this node's layer
                if (!layerOf.TryGetValue(chain[i].Id, out int li)) continue;
                var layer = layers[li];
                int idx = layer.IndexOf(chain[i].Id);
                if (idx < 0) continue;

                double newLeft = chain[i].X + shift;
                double newRight = newLeft + chain[i].Width;

                bool ok = true;
                // Skip dummies — find nearest REAL neighbor for overlap check
                for (int j = idx - 1; j >= 0; j--)
                {
                    var ln = nodes[layer[j]];
                    if (ln.IsDummy) continue;
                    if (newLeft < ln.X + ln.Width + NodeGapX)
                        ok = false;
                    break;
                }
                for (int j = idx + 1; j < layer.Count; j++)
                {
                    var rn = nodes[layer[j]];
                    if (rn.IsDummy) continue;
                    if (newRight + NodeGapX > rn.X)
                        ok = false;
                    break;
                }

                if (ok) chain[i].X += shift;
            }
        }

        // Step B: Align multi-child parents with first logical child
        foreach (var n in nodes.Values)
        {
            if (n.IsDummy) continue;
            var realChildren = n.Children
                .Where(cid => nodes.TryGetValue(cid, out var c) && !c.IsDummy)
                .Select(cid => nodes[cid])
                .ToList();
            if (realChildren.Count < 2) continue;

            // Find first child by DisplayIndex
            var firstChild = realChildren.OrderBy(c => c.DisplayIndex).First();
            double parentCenter = n.X + n.Width / 2;
            double childCenter = firstChild.X + firstChild.Width / 2;
            double shift = childCenter - parentCenter;
            if (Math.Abs(shift) < 1) continue;

            // Check overlap constraints in parent's layer
            if (!layerOf.TryGetValue(n.Id, out int pli)) continue;
            var pLayer = layers[pli];
            int pIdx = pLayer.IndexOf(n.Id);
            if (pIdx < 0) continue;

            double newLeft = n.X + shift;
            double newRight = newLeft + n.Width;

            bool ok = true;
            // Skip dummies — find nearest REAL neighbor for overlap check
            for (int j = pIdx - 1; j >= 0; j--)
            {
                var ln = nodes[pLayer[j]];
                if (ln.IsDummy) continue;
                if (newLeft < ln.X + ln.Width + NodeGapX)
                    ok = false;
                break;
            }
            for (int j = pIdx + 1; j < pLayer.Count; j++)
            {
                var rn = nodes[pLayer[j]];
                if (rn.IsDummy) continue;
                if (newRight + NodeGapX > rn.X)
                    ok = false;
                break;
        ...
        CultureInfo ci)
    {
        ...
        if (n.HeaderLines == 2)
        {
            // 2-line header: index centered vertically, title on two lines
            double idxY = y + headerH / 2 + FontSzHeader * 0.35;
            ...
            double line1Y = y + headerH / 2 - FontSzTitle * 0.25;
        ...
        {
            double headerTextY = y + headerH / 2 + Math.Max(FontSzHeader, FontSzTitle) * 0.35;
        ...
        if (hasReward)
        {
            ...
            if (hasXp)
            {
                // XP reward: "number ⭐ · Item Reward" — XP number, icon, then optional item
                double rewardY = cy;
                double rewardTextY = rewardY + IconSize / 2 + FontSzReward * 0.35;
                ...
                // XP icon
                double iconY = rewardY + (IconSize - IconSize) / 2; // vertically centered
                sb.AppendLine($"    <image x=\"{cursorX.ToString(ci)}\" y=\"{rewardY.ToString(ci)}\"" +
        ...
        double ox, double oy, CultureInfo ci)
    {
        ...
        foreach (var (fromId, toId) in realEdges)
        {
            ...
            path.Append($"M {Fmt(sx, ci)},{Fmt(sy, ci)}");
            ...
            if (isMultiChild)
            {
                ...
                path.Append($" L {Fmt(sx, ci)},{Fmt(busY, ci)}");
                routeStartY = busY;
            }
            ...
            if (Math.Abs(routeStartX - tx) < SnapThresh)
            {
                ...
                {
                    path.Append($" L {Fmt(routeStartX, ci)},{Fmt(ty, ci)}");
                    sb.AppendLine($"    <path class=\"edge-path\" d=\"{path}\" marker-end=\"url(#arrow)\" />");
                    continue;
                }
            }
            ...
            if (srcLayer >= 0 && tgtLayer >= 0 && tgtLayer > srcLayer)
            {
                for (int gapLayer = srcLayer; gapLayer < tgtLayer; gapLayer++)
                {
                    ...
                    if (!hit)
                    {
                        path.Append($" L {Fmt(routeStartX, ci)},{Fmt(gapY, ci)}");
                        path.Append($" L {Fmt(tx, ci)},{Fmt(gapY, ci)}");
                        path.Append($" L {Fmt(tx, ci)},{Fmt(ty, ci)}");
                        routed = true;
                        break;
                    }
                }
            ...
            if (!routed)
            {
                // Strategy 3: Corridor routing (2 horizontal segments + vertical corridor)
                double corridorX = FindClearCorridor(routeStartX, tx, nodeBoxes,
                    layerTop, layerBottom, srcLayer, tgtLayer, fromId, toId, margin, ox);

                // Use first gap below source and last gap above target
                double gapY1 = routeStartY;
                double gapY2 = ty;

                if (srcLayer >= 0 && tgtLayer > srcLayer)
                {
                    gapY1 = (layerBottom[srcLayer] + layerTop[Math.Min(srcLayer + 1, layers.Count - 1)]) / 2;
                    if (gapY1 <= routeStartY + 2) gapY1 = routeStartY + BusOffset;
                    gapY2 = (layerBottom[Math.Max(tgtLayer - 1, 0)] + layerTop[tgtLayer]) / 2;
                    if (gapY2 >= ty - 2) gapY2 = ty - BusOffset;
                }

                if (Math.Abs(routeStartX - corridorX) < SnapThresh &&
                    Math.Abs(corridorX - tx) < SnapThresh)
                {
                    // All aligned — straight line
                    path.Append($" L {Fmt(routeStartX, ci)},{Fmt(ty, ci)}");
                }
                else if (Math.Abs(routeStartX - corridorX) < SnapThresh)
                {
                    // Source aligned with corridor — just bend at gapY2 to target
                    path.Append($" L {Fmt(routeStartX, ci)},{Fmt(gapY2, ci)}");
                    path.Append($" L {Fmt(tx, ci)},{Fmt(gapY2, ci)}");
                    path.Append($" L {Fmt(tx, ci)},{Fmt(ty, ci)}");
                }
                else if (Math.Abs(corridorX - tx) < SnapThresh)
                {
                    // Corridor aligned with target — bend at gapY1 to corridor/target
                    path.Append($" L {Fmt(routeStartX, ci)},{Fmt(gapY1, ci)}");
                    path.Append($" L {Fmt(tx, ci)},{Fmt(gapY1, ci)}");
                    path.Append($" L {Fmt(tx, ci)},{Fmt(ty, ci)}");
                }
                else
                {
                    // Full corridor: source → gapY1 → corridorX → gapY2 → target
                    path.Append($" L {Fmt(routeStartX, ci)},{Fmt(gapY1, ci)}");
                    path.Append($" L {Fmt(corridorX, ci)},{Fmt(gapY1, ci)}");
                    path.Append($" L {Fmt(corridorX, ci)},{Fmt(gapY2, ci)}");
                    path.Append($" L {Fmt(tx, ci)},{Fmt(gapY2, ci)}");
                    path.Append($" L {Fmt(tx, ci)},{Fmt(ty, ci)}");
                }
            }
    ...
    /// <summary>
    /// Finds a clear vertical X corridor through intermediate layers that doesn't
    /// intersect any node. Picks the X closest to the midpoint of (sx, tx).
    /// </summary>
    private static double FindClearCorridor(double sx, double tx,
        List<(double left, double top, double right, double bottom, string id)> nodeBoxes,
        double[] layerTop, double[] layerBottom,
        int srcLayer, int tgtLayer,
        string sourceId, string targetId, double margin, double ox)
    {
        double midX = (sx + tx) / 2;

        // Collect all blocked X ranges from nodes in intermediate layers
        var blocked = new List<(double left, double right)>();
        foreach (var (l, t, r, b, id) in nodeBoxes)
        {
            if (id == sourceId || id == targetId) continue;
            // Check if this node is in an intermediate layer (between srcLayer and tgtLayer)
            if (srcLayer >= 0 && tgtLayer >= 0 && t < layerBottom[tgtLayer] && b > layerTop[Math.Min(srcLayer + 1, layerTop.Length - 1)])
                blocked.Add((l - margin, r + margin));
        }

        // Sort blocked ranges
        blocked.Sort((a, b) => a.left.CompareTo(b.left));

        // If no blocked ranges, use midpoint
        if (blocked.Count == 0) return midX;

        // Find free X closest to midX
        // Build merged blocked intervals
        var merged = new List<(double left, double right)>();
        foreach (var rng in blocked)
        {
            if (merged.Count > 0 && rng.left <= merged[^1].right)
                merged[^1] = (merged[^1].left, Math.Max(merged[^1].right, rng.right));
            else
                merged.Add(rng);
        }

        // Check if midX is free
        bool midFree = true;
        foreach (var (l, r) in merged)
        {
            if (midX >= l && midX <= r) { midFree = false; break; }
        }
        if (midFree) return midX;

        // Find nearest free X to midX (check edges of merged intervals)
        double bestX = midX;
        double bestDist = double.MaxValue;

        foreach (var (l, r) in merged)
        {
            // Left edge of blocked range
            double candL = l - 1;
            double distL = Math.Abs(candL - midX);
            if (distL < bestDist)
            {
                // Verify this candidate is actually free
                bool free = true;
                foreach (var (l2, r2) in merged)
                    if (candL >= l2 && candL <= r2) { free = false; break; }
                if (free) { bestX = candL; bestDist = distL; }
            }

            // Right edge of blocked range
            double candR = r + 1;
            double distR = Math.Abs(candR - midX);
            if (distR < bestDist)
            {
                bool free = true;
                foreach (var (l2, r2) in merged)
                    if (candR >= l2 && candR <= r2) { free = false; break; }
                if (free) { bestX = candR; bestDist = distR; }
            }
        }
