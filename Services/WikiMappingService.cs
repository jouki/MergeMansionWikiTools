...
public static class WikiMappingService
{
    ...
    /// <summary>
    /// Queries the wiki for existing <c>Module:Datatable/Dialogues/*</c> chunk numbers, split into the
    /// main-story subtree (<c>/1</c>, <c>/2</c>, …) and the events subtree (<c>/Events/1</c>, …). One
    /// <c>allpages</c> query covers both (and skips non-numeric pages like <c>/Archive</c>) — the two
    /// subtrees share a title prefix, unlike Areas/Items which only ever had one.
    /// </summary>
    public static async Task<(List<int> Main, List<int> Events)> QueryExistingDialogueModulesAsync()
    {
        ...
        {
            var url = $"{BaseApiUrl}?action=query&list=allpages" +
                      "&apprefix=Datatable/Dialogues/&apnamespace=828&aplimit=100&format=json";
            if (apcontinue != null)
                url += $"&apcontinue={Uri.EscapeDataString(apcontinue)}";
                ...
                && query.TryGetProperty("allpages", out var pages))
            {
                ...
                {
                    var title = page.GetProperty("title").GetString() ?? "";
                    var em = Regex.Match(title, @"^Module:Datatable/Dialogues/Events/(\d+)$");
                    if (em.Success) { events.Add(int.Parse(em.Groups[1].Value)); continue; }
                    var mm = Regex.Match(title, @"^Module:Datatable/Dialogues/(\d+)$");
                    if (mm.Success) main.Add(int.Parse(mm.Groups[1].Value));
                    // anything else (Archive, Unassigned/N, …) is not a chunk this updater manages
                }
            }
    ...
    /// <summary>
    /// Generates the dispatcher ("rozcestník") <c>Module:Datatable/Dialogues</c>: requires every main
    /// and event chunk and flat-merges their scene maps into <c>p.scenes</c>, the same require+merge
    /// shape as <see cref="GenerateItemsArbiterLua"/> — a page-building consumer needs only this one
    /// module instead of knowing chunk counts/boundaries. Unlike Areas/Items chunks (which nest their
    /// data under a named field), a Dialogues chunk's <c>return {...}</c> IS the flat scene map already
    /// (see <c>LuaGeneratorService.RenderDialogueModule</c>), so the merge loop needs no field name.
    /// </summary>
    public static string GenerateDialoguesArbiterLua(int mainChunkCount, int eventChunkCount, string? createdAt)
    {
        var sb = new StringBuilder();
        sb.Append(LuaGeneratorService.BuildHeader(createdAt));
        sb.AppendLine("-- Module:Datatable/Dialogues (dispatcher). Generated by MergeMansionWikiTools; do not edit by hand.");
        sb.AppendLine("local p = {}");
        sb.AppendLine("p.scenes = {}");
        for (int i = 1; i <= mainChunkCount; i++)
            sb.AppendLine($"for k, v in pairs(require('Module:Datatable/Dialogues/{i}')) do p.scenes[k] = v end");
        for (int i = 1; i <= eventChunkCount; i++)
            sb.AppendLine($"for k, v in pairs(require('Module:Datatable/Dialogues/Events/{i}')) do p.scenes[k] = v end");
        sb.AppendLine();
        sb.AppendLine("return p");
        return sb.ToString();
    }
