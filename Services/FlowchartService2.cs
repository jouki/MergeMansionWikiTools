...
/// VERSION 2 — working copy for improvements (original FlowchartService kept as backup).
/// </summary>
internal class FlowchartService2
{
    // ── Internal models ──────────────────────────────────────────────
        ...
        foreach (var (fromId, toId) in realEdges)
        {
            ...
            if (!routed)
            {
                ...
                if (sameColumn)
                {
                    ...
                    // Z-bend degenerates to a straight line here, so we must jog sideways.
                    // Minimal corridor: down → jog left/right → down → jog back → down.
                    double jogOffset = MaxNodeW / 2 + NodeGapX;

                    // Try left first, then right — pick the side with fewer collisions
                    double jogLeft = routeStartX - jogOffset;
                    double jogRight = routeStartX + jogOffset;

                    // Compute gap Y positions (first gap below source, last gap above target)
                    ...
                    // Score each side: count segment collisions
                    ...
                    path.Append($" L {Fmt(jogX, ci)},{Fmt(gapY1, ci)}");
                    path.Append($" L {Fmt(jogX, ci)},{Fmt(gapY2, ci)}");
                ...
                else
                {
                    ...
                    // Find a clear vertical corridor X between node columns and route:
                    // source↓ → horizontal→corridorX → corridor↓ → horizontal→target → target↓
                    ...
                    // Collect X intervals occupied by nodes in the gapY1..gapY2 vertical range
                    ...
                    // Build candidate corridor X positions from gaps between occupied bands
                    ...
                    // Score each candidate: full 5-segment collision check
                    ...
                    // Emit corridor route (collapse when corridor aligns with source/target)
                    ...
                    {
                        path.Append($" L {Fmt(routeStartX, ci)},{Fmt(gapY1, ci)}");
                        path.Append($" L {Fmt(bestCX, ci)},{Fmt(gapY1, ci)}");
                        path.Append($" L {Fmt(bestCX, ci)},{Fmt(gapY2, ci)}");
                        path.Append($" L {Fmt(tx, ci)},{Fmt(gapY2, ci)}");
                        path.Append($" L {Fmt(tx, ci)},{Fmt(ty, ci)}");
                    }
                }
