    ...
    internal static class GameConfigUtil
    {
            ...
            where TValue : IGameConfigData<TKey>
        {
            ...
            catch (Exception e)
            {
                ...
                Console.WriteLine($"[ERROR] Deserializing table {typeof(TValue).Name}: {inner.Message}");
                Console.WriteLine($"[TRACE] {inner.GetType().FullName}: {inner.StackTrace}");
                return null;
            }
        }
        ...
        public static T ImportBinaryKeyValueStructure<T>(IConfigArchive archive, string fileName)
        {
            ...
            {
                var inner = e.InnerException ?? e;
                Console.WriteLine($"[ERROR] Deserializing structure {typeof(T).Name}: {inner.Message}");
                Console.WriteLine($"[TRACE] {inner.GetType().FullName}: {inner.StackTrace}");
                return default;
            }
        }
