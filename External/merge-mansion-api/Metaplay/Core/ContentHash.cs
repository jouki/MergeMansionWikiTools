// Modified by Jouki (2026) — 3-segment hash format support for v26+ language files
using System;
    ...
    public struct ContentHash
    {
        ...
        public static ContentHash ParseString(string str)
        {
            if (str == null)
                return None;
            var parts = str.Split('-');
            // Support both 2-segment (High-Low) and 3-segment (Prefix-High-Low) hash formats
            ...
            if (parts.Length == 3 && parts[1].Length == 0x10 && parts[2].Length == 0x10)
            {
                // 3-segment format: first segment is extra metadata, use last two as High/Low
                var p1 = Convert.ToUInt64(parts[1], 16);
                var p2 = Convert.ToUInt64(parts[2], 16);
                return new ContentHash(new MetaUInt128(p1, p2));
            }
