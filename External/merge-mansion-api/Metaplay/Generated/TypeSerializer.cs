...
namespace Metaplay.Generated
{
    public static class TypeSerializer
    {
        ...
        private static void Deserialize_WireType(ref MetaSerializationContext context, IOReader reader, WireDataType wireType, Type memberType, out object value)
        {
            ...
            switch (wireType)
            {
                ...
                case WireDataType.AbstractStruct:
                    ...
                    var targetType = TryGetDerivableType(memberType, deriveId);
                    if (targetType == null)
                    {
                        // Unknown derivative — skip the struct data, leave value as null
                        Console.WriteLine($"[WARN] Unknown derivative {deriveId} for type {memberType.FullName}, skipping.");
                        TaggedWireSerializer.SkipStructMembers(reader);
                        return;
                    }
        ...
        private static Type TryGetDerivableType(Type baseType, int targetId)
        {
            if (baseType == MetaMessageType)
                return GetMetaMessageType(baseType, targetId);

            return TryGetSerializableDerivedType(baseType, targetId);
        }
        ...
        private static Type TryGetSerializableDerivedType(Type baseType, int targetId)
        {
            EnsureDerivableTypesCache(baseType);
            _derivableTypes[baseType].TryGetValue(targetId, out var derivableType);
            return derivableType; // null if not found
        }
        ...
        private static Type GetSerializableDerivedType(Type baseType, int targetId)
        {
            ...
            if (!_derivableTypes[baseType].TryGetValue(targetId, out var derivableType))
