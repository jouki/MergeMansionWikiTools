...
## Portraits in the viewer (`build/portraits.py`, `_cache/portraits/`)

Character portraits per emotion state are cropped from the user's asset export (`APKs/<ver>/image_atlas_data.json` sprite rects + `Export - PNGs/<textureName>.png`, newest version first, older versions as fallback). Sprite names: `MaddieWorried`, `McLeod_Doubtful`, `Ash-Default`. Build 2026-09-05: 287 portraits for 31 of 46 characters (20 states). Missing: the Jailbreak cast (Grace, Bluetooth, Warden Decker, …) — their sprites were only in 25.05–25.06 builds for which no atlas export exists — plus Dog, Pet, Phone. The viewer shows every spoken line as in the game: left portrait, speech, right portrait; the speaker's side carries the line's emotion state, the other side is dimmed in its Default pose; `NoChange` on a side keeps the previous line's character.
