...
using Upstream = merge_mansion_dumper.Dumper;   // the ONLY using of the upstream namespace in the harness

namespace DumpHarness;

/// <summary>
/// Harness-local copy of Services/Dumping/LegacyDumpEngine.cs. The harness must not reference the
/// WPF app, so the temporary Legacy adapter is deliberately duplicated here; both copies disappear
/// in phase 2 when External/merge-mansion-dumper is removed. Keep the two in sync until then.
/// (Nullable is disabled in this project, so the '?' annotations are stripped.)
/// </summary>
internal sealed class LegacyDumpEngineHarness : IDumpEngine
{
    ...
    public DumpOutput DumpCardCollection(SharedGameConfig config, string path)
        => new DumpOutput(new Upstream.CardCollectionDumper().WriteJson(path, config), true);

    public DumpOutput DumpDialogues(SharedGameConfig config, string path)
        => new DumpOutput(new Upstream.DialogueDumper().WriteJson(path, config), true);
    ...
    {
        if (baseline == null) return new DumpOutput(writeJson(path), true);
        var written = writeJsonIfDifferent(path, baseline);
        return new DumpOutput(written ? File.ReadAllText(path) : baseline, written);
    }
    ...
    internal static Upstream.EventFilters MapFilters(EventFilters f)
    {
        ...
        foreach (EventFilters v in Enum.GetValues(typeof(EventFilters)))
        {
            ...
            else Console.WriteLine($"[WARN] EventFilters.{v} has no upstream equivalent — ignored");
        }
        return r;
    }
}
